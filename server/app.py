...
async def select_branch():
    try:
        ...
        if not user_id:
            return jsonify({'status': 'error', 'message': 'ID de utilizador não fornecido.'}), 400
...
def get_all_data():
    try:
        ...
        if not user_id or user_id not in user_data:
            return jsonify({
                'status': 'error',
                'message': 'Sessão de utilizador não encontrada. Por favor atualize a página.'
            }), 400
        ...
        return jsonify({
            ...
            'machines': machines,
            'production_orders': ln_production_orders
        })
    except Exception as e:
        print(f"Error in getting chart data: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Erro ao carregar dados do gráfico. Por favor tente novamente.'
        }), 500
...
def get_results_data():
    try:
        ...
        return jsonify({
            'status': 'success',
            'new_exec_plans': new_exec_plans,
            'time_units': time_units,
            'machines': machines,
            'production_orders': ln_production_orders
        })
    except Exception as e:
        print(f"Error in getting new chart data: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Erro ao carregar dados do específico plano. Por favor tente novamente.'
        }), 500
...
def upload_file():    
    try:
        ...
        if not file or not allowed_file(file.filename):
            return jsonify({
                'status': 'error',
                'message': 'O formato do ficheiro é inválido. Por favor selecione um ficheiro Excel (.xlsx).'
            }), 400
        ...
        is_valid = validate_file_structure(plan_file_path)
        if not is_valid:
            os.remove(plan_file_path)
            return jsonify({
                'status': 'error',
                'message': 'Ficheiro com estrutura incorreta, tente outra vez.'
            }), 400
        ...
        return jsonify({
            'status': 'success',
            'message': 'Ficheiro lido e guardado com sucesso.',
        ...
        return jsonify({
            'status': 'error',
            'message': 'Erro no upload do ficheiro. Por favor tente novamente.'
        }), 500
...
def delete_file():     
    try:
        ...
        return jsonify({'status': 'success', 'message': 'Ficheiro de entrada eliminado com sucesso.'}), 200
    except Exception as e:
        print(f"Error in input file deletion: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Erro na eliminação do ficheiro de entrada.'
        }), 500
...
def process_criteria():
    try: 
        ...
        if not data:
            return jsonify({
                'status': 'error',
                'message': 'Dados em falta no pedido.'
            }), 400
        
        selected_criteria = data.get('selectedCriteria')
        criteria = data.get('allCriteria')
        
        if not selected_criteria or not criteria:
            return jsonify({
                'status': 'error',
                'message': 'Critérios em falta.'
            }), 400
        ...
        return jsonify({
            'status': 'error',
            'message': 'Erro no processamento dos critérios. Por favor tente novamente.'
        }), 500
...
def get_machines():
    try:
        ...
        print(f"Error in machine fetching: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Erro ao obter máquinas. Por favor tente novamente.'
        }), 500
...
def get_BoMs():
    try:
        ...
        print(f"Error in BoM fetching: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Erro ao obter BOMs. Por favor tente novamente.'
        }), 500
...
def remove_machines():
    try:
        ...
        return jsonify({'status': 'success', 'message': 'As máquinas escolhidas foram removidas com sucesso.'}), 200 # A success message is returned, if the machines are removed successfully.
    except Exception as e:
        print(f"Error in machine removal: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Erro ao remover máquinas. Por favor tente novamente.'
        }), 500
...
def remove_boms():
    try:
        ...
        return jsonify({'status': 'success', 'message': 'As BoMs escolhidas foram removidas com sucesso.'}), 200 # A success message is returned, if the BoM's are removed successfully.
    except Exception as e:
        print(f"Error in BoM removal: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Erro ao remover BOMs. Por favor tente novamente.'
        }), 500
...
def create_data():
    try:
        ...
        # Clear the user-specific previous data instances
        ...
        return jsonify({
            'status': 'success',
            'message': 'Ocorreu um erro na criação de dados. Por favor tente novamente.',
            'no_routings': no_routings,
            'no_bom': no_bom,
        }), 200
    except Exception as e:
        print(f"Error in data creation: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Ocorreu um erro na criação de dados. Por favor tente novamente.'
        }), 500
...
def run_algorithm_in_thread(user_id, dataHandler, PT_Settings):
    ...
    try:
        ...
        running_algorithms[user_id]["message"] = "Ocorreu um erro na execução do algoritmo. Por favor tente novamente."
...
def run_algorithm():
    try:
        ...
        if executor._work_queue.qsize() >= executor._max_workers:
            return jsonify({
                'status': 'pool_full',
                'message': 'Sistema encontra-se em capacidade máxima. Por favor, tente novamente mais tarde.'
            }), 503  
        ...
        return jsonify({
            'status': 'error',
            'message': 'Erro ao iniciar algoritmo. Por favor tente novamente.'
        }), 500
...
def algorithm_status():
    try:
        ...
        if running_algorithms[user_id]["status"] in ["completed", "error", "aborted"]:
        ...
        return jsonify({
            'status': running_algorithms[user_id]["status"],
            'message': running_algorithms[user_id]["message"],
            'late_orders': running_algorithms[user_id]["late_orders"]
        }), 202 
        
    except Exception as e:
        print(f"Error in algorithm status: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Erro ao obter estado do algoritmo. Por favor tente novamente.'
        }), 500
...
def plan_history():
    try:
        ...
        print(f"Error in getting plan history: {e}")
        return jsonify({
            'status': 'error',
            'message': 'Erro ao obter histórico de planos. Por favor tente novamente.'
        }), 500
